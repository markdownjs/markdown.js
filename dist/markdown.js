...
(function(mod) {
  if (typeof exports == "object" && typeof module == "object") // CommonJS
	...
	var block = {
	  newline: /^\n+/,
	  code: /^( {4}[^\n]+\n*)+/,
	  fences: markdown.noop,
	  hr: /^( *[-*_]){3,} *(?:\n+|$)/,
	  heading: /^ *(#{1,6}) *([^\n]+?) *#* *(?:\n+|$)/,
	  nptable: markdown.noop,
	  lheading: /^([^\n]+)\n *(=|-){2,} *(?:\n+|$)/,
	  blockquote: /^( *>[^\n]+(\n(?!def)[^\n]+)*\n*)+/,
	...
	block.bullet = /(?:[*+-]|\d+\.)/;
	block.item = /^( *)(bull) [^\n]*(?:\n(?!\1bull )[^\n]*)*/;
	block.item = markdown.replace(block.item, 'gm')
	  (/bull/g, block.bullet)
	  ();

	block.list = markdown.replace(block.list)
	  (/bull/g, block.bullet)
	  ('hr', '\\n+(?=\\1?(?:[-*_] *){3,}(?:\\n+|$))')
	  ('def', '\\n+(?=' + block.def.source + ')')
	  ();

	block.blockquote = markdown.replace(block.blockquote)
	  ('def', block.def)
	  ();
	...
	block.html = markdown.replace(block.html)
	  ('comment', /<!--[\s\S]*?-->/)
	  ('closed', /<(tag)[\s\S]+?<\/\1>/)
	  ('closing', /<tag(?:"[^"]*"|'[^']*'|[^'">])*?>/)
	  (/tag/g, block._tag)
	...
	block.paragraph = markdown.replace(block.paragraph)
	  ('hr', block.hr)
	...
	block.normal = markdown.merge({}, block);

	/**
	 * GFM Block Grammar
	 */

	block.gfm = markdown.merge({}, block.normal, {
	  fences: /^ *(`{3,}|~{3,})[ \.]*(\S+)? *\n([\s\S]*?)\s*\1 *(?:\n+|$)/,
	  paragraph: /^/,
	  heading: /^ *(#{1,6}) +([^\n]+?) *#* *(?:\n+|$)/
	});

	block.gfm.paragraph = markdown.replace(block.paragraph)
	  ('(?!', '(?!'
	...
	block.tables = markdown.merge({}, block.gfm, {
	  nptable: /^ *(\S.*\|.*)\n *([-:]+ *\|[-| :]*)\n((?:.*\|.*(?:\n|$))*)\n*/,
	  table: /^ *\|(.+)\n *\|( *[-:]+[-| :]*)\n((?: *\|.*(?:\n|$))*)\n*/
	});
	...
	inline.link = markdown.replace(inline.link)
	  ('inside', inline._inside)
	  ('href', inline._href)
	  ();

	inline.reflink = markdown.replace(inline.reflink)
	  ('inside', inline._inside)
	  ();
	...
	inline.normal = markdown.merge({}, inline);

	/**
	 * Pedantic Inline Grammar
	 */

	inline.pedantic = markdown.merge({}, inline.normal, {
	...
	inline.gfm = markdown.merge({}, inline.normal, {
	  escape: markdown.replace(inline.escape)('])', '~|])')(),
	  url: /^(https?:\/\/[^\s<]+[^<.,:;"')\]\s])/,
	  del: /^~~(?=\S)([\s\S]*?\S)~~/,
	  text: markdown.replace(inline.text)
	    (']|', '~]|')
	    ('|', '|https?://|')
	    ()
	});
	...
	inline.breaks = markdown.merge({}, inline.gfm, {
	  br: markdown.replace(inline.br)('{2,}', '*')(),
	  text: markdown.replace(inline.gfm.text)('{2,}', '*')()
	});
...
(function(mod) {
  if (typeof exports == "object" && typeof module == "object") // CommonJS
	 ...
	 * Block Lexer
	 **************************/
	var block = markdown.grammar.block; 
